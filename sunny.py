...
def ngrok_auth(options):
    ...
    try:
        ...
        context.check_hostname = False  # 如果需要证书验证，请启用 check_hostname 并使用 context.load_verify_locations()
        context.verify_mode = ssl.CERT_NONE  # 如果需要验证证书，请使用 ssl.CERT_REQUIRED 并加载 CA 证书
